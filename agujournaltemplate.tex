...
\title{Automatic input variable selection with genetic algorithms for precipitation downscaling}
...
\begin{table}
	...
	\begin{tabular}{clcc}
		...
		11 & Weisse Lütschine & 165.0 & 2149 \\
		12 & Emme & 206.9 & 1151 \\
...
An variant of genetic algorithms (GAs) has been tailored for the optimization of AMs by \citeA{Horton2017a}. All the parameters of the method but the meteorological variables and the analogy criteria could be successfully optimized using GAs \cite{Horton2018a}. The use of GAs provided for the first time an objective and global optimization of AMs, which resulted in gain in prediction skills. To bring the optimization further, the selection of the predictor variables and the analogy criteria were here performed by GAs. 
...
The mutation operator having the largest impact on the success of the optimization, five variants, as suggested in \citeA{Horton2017a}, were used. Thus, every parameter set was optimized five to seven times with different GAs options.
...
\subsection{Changes Made to AtmoSwing}

Despite being a simple method, the AM require many comparisons of gridded fields for its calibration. For example, this work used a 24-years calibration period. For each target day, a gridded predictor needs to be compared to about 2820 candidate situations (on a 120-days temporal window, minus 60 days in the target year). Over the full calibration period, this amounts to about $24.7\cdot10^6$ field comparisons per predictor of the first level of analogy. Here, one optimization required 100-300 generations made of 2000 individuals, which brings the average number of grid comparisons to about $1\cdot10^{13}$ per predictor of the first level of analogy. Using profilers, the comparison of the gridded predictors was clearly identified as the most time-consuming task, despite the use of the efficient linear algebra library Eigen 3 \cite{Guennebaud2010}.

In order to reduce the number of comparisons and the processing time, the following changes were implemented in the release of AtmoSwing v.2.1.2 \cite{Horton2019b} :
\begin{itemize}
	\item When a new parameter set is generated, it is compared to all previously assessed parameters to check if it has already been previously assessed. If this is the case, the skill score previously processed is assigned to the new parameter set.
	\item Similarly, if a parameter set close to the new one has been previously assessed in former generations and was found far from the region of the optimum, its skill score is also recycled. The actual skill score of the new parameter set is likely to be different, but this is reasonable as this under-performing parameter set will be quickly discarded in the evolution process. The constraints for similarities are: same predictor variables, vertical levels, time, analogy criteria, a difference in the number of analogs inferior to 10\%, slight variations of the spatial windows, and a difference in the weight of maximum 0.1. Under-performing parameter sets are defined as inferior to the median of the full history.
	\item The comparison of the predictor grids can now be done using graphics processing unit (GPU). More details on this implementation is provided in the next section.
	\item The optimizations were made fully resumable by saving and restoring the state of the operators of the GAs.
\end{itemize}


\subsection{Graphics Processing Units (GPUs) Computing}

Comparison of gridded data is a calculation that can scale well on graphics processing units (GPUs). The calculation of the analogy criteria has thus been written using NVIDIA's CUDA. The different criteria available in AtmoSwing were written as kernels that run on the GPU.
...
%TODO: selected variables & analogy criteria
%TODO: check the selected domains
%TODO: illustrate e.g. S2 for 1 event (maps).
...
%TODO: matrix of n levels of analogy x n predictors (for limited amount of catchments)
%TODO: assess gains vs overfitting
%TODO: assess most performing GAs operators


\subsection{Best Variables Combination}
\label{best_multi}

%TODO: optimize with relevant structure for all catchments
%TODO: remove criteria and variables that were not selected before ? Adjust max domain ?
%TODO: try to build 1 common AM for all catchments (same predictors, different weights)
%TODO: compare skill with existing AMs
