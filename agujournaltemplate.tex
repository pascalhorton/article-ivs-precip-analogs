...
These results show again a strong dominance of the S0, S1 and S2 analogy criteria, with the others being only marginally selected, including RMSE. S0, which represents a comparison of the grid values, is most often selected. The properties of S0 are further investigated in Sect. \ref{discussion_S0}.  
...
CAPE is the third most selected variable and the total column water (TCW) is the sixth variable. At the nineth position comes the meridional wind at 10~m, but compared in a derived way using S1 or even S2. Then comes the meridional wind on the PV level. The 2~m temperature has the 12th position and is compared in terms of gradients (S1). Follows the geopotential height (Z) at 700 and 600~hPa compared primarily using the second derivatives of the fields (S2). SLP, a bit further down on the list, is also compared in terms of its second derivative. Other variables such as RH, PV, D, and U also populate the 30 best variables.
