...
In order to assess the relevance of the methods optimized in this work, they have been compared to the reference methods (Sect. \ref{ams}). Figure \ref{fig_scores} shows the improvement of the CRPS score for the different reference and resulting methods compared to the simplest 2Z method. The CRPS values being heavily influenced by the climatology, and thus significantly different from one station to another, they are best compared relatively to a reference stationwise.

\begin{figure}[hbt]
	\noindent\makebox[\textwidth]{
		\includegraphics[width=120mm]{figures/scores-all-methods.pdf}
	}
	\caption{Performance score of the different reference and optimized methods on the validation period for the 25 catchments. The skill score is expressed as a percentage improvement (lower values) in terms of the CRPS when considering 2Z as a benchmark. Larger improvement is better.}
	\label{fig_scores}
\end{figure}

One can see in Fig. \ref{fig_scores} that the selection of a single best variable (GAs 1) already achives better results than the 2Z method. Obviously, the performance provided by a single variable remains lower than the more complex AMs. When considering more variables, the optmized methods perform significantly better than the reference methods, particularly the GAs 1~x~8 and GAs 1~x~12. Thus, despite having a single level of analogy, they outperform complex stepwise AMs. The selection of the predictor variables and the analogy criteria by GAs, along with all other parameters, provide AMs that prove to be relevant also on the validation period.
